...
```text
Use the Okta IT Operations MCP server to look up eng.employee@example.com.
Summarize the user's Okta status, department, user type, and last login.
```

```text
List the Okta groups for eng.employee@example.com and explain what access those groups probably grant.
```

```text
Search the Okta System Log for recent sign-in and MFA events for eng.employee@example.com.
...
```text
Explain whether eng.employee@example.com has Slack access and why.
Focus on whether access comes from app-default-users or another Okta group.
```

```text
Explain whether contractor.user@example.com has Slack access and whether that access path looks appropriate for a contractor.
```
